        ...
        @if ($terpilih)
            <div class="space-y-5">
                <div class="grid gap-3 sm:grid-cols-2">
                    @foreach ([['Kode', $terpilih->kode], ['Penyewa', $terpilih->nama], ['WhatsApp', $terpilih->whatsapp], ['Email', $terpilih->email ?: '—'], ['Kendaraan', $terpilih->nama_kendaraan], ['Transmisi', $terpilih->transmisi], ['Lama sewa', $terpilih->durasi_label], ['Mulai', $terpilih->tanggal_mulai->translatedFormat('j F Y') . ' pukul ' . substr((string) $terpilih->jam_mulai, 0, 5)], ['Sopir', $terpilih->dengan_sopir ? 'Dengan sopir' : 'Lepas kunci'], [$terpilih->dengan_sopir ? 'Titik penjemputan' : 'Lokasi pengantaran unit', $terpilih->lokasi_antar ?: '—'], [$terpilih->dengan_sopir ? 'Tujuan' : 'Lokasi pengembalian unit', ($terpilih->dengan_sopir ? $terpilih->tujuan : $terpilih->lokasi_kembali) ?: '—'], ['Estimasi biaya', $rupiah($terpilih->estimasi_biaya)], ['Tarif unit saat ini', $terpilih->kendaraan ? $rupiah($terpilih->kendaraan->tarif($terpilih->satuan)) . ' / ' . ($terpilih->satuan_label) : '—']] as [$label, $nilai])
                        <div>
